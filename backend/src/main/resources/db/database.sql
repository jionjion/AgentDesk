...
-- 初始化模型数据
INSERT INTO agent_desk.model_definitions (id, display_name, group_name, input_modalities, supports_reasoning,
                                          max_context_window, max_output_tokens, description, sort_order, enabled,
                                          created_at, updated_at)
VALUES
    -- 旗舰模型
    ('qwen3.6-plus', 'Qwen3.6 Plus', '旗舰模型', '[
      "text",
      "image",
      "video"
    ]', TRUE, 1000000, 65536, '最新旗舰，多模态推理', 100, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen3.5-plus', 'Qwen3.5 Plus', '旗舰模型', '[
      "text",
      "image",
      "video"
    ]', TRUE, 1000000, 65536, '上代旗舰多模态', 101, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen3-max', 'Qwen3 Max', '旗舰模型', '[
      "text"
    ]', TRUE, 262144, 65536, '文本推理最强', 102, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen-plus', 'Qwen Plus', '旗舰模型', '[
      "text"
    ]', TRUE, 1000000, 32768, '高性价比', 103, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    -- 轻量快速
    ('qwen3.5-flash', 'Qwen3.5 Flash', '轻量快速', '[
      "text",
      "image",
      "video"
    ]', TRUE, 1000000, 65536, '快速多模态', 200, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen-flash', 'Qwen Flash', '轻量快速', '[
      "text"
    ]', TRUE, 1000000, 32768, '极速文本', 201, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen-turbo', 'Qwen Turbo', '轻量快速', '[
      "text"
    ]', FALSE, 1000000, 8192, '最快响应', 202, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    -- 代码专精
    ('qwen3-coder-plus', 'Qwen3 Coder Plus', '代码专精', '[
      "text"
    ]', FALSE, 1000000, 65536, '代码生成 Plus', 300, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen3-coder-flash', 'Qwen3 Coder Flash', '代码专精', '[
      "text"
    ]', FALSE, 1000000, 65536, '代码生成 Flash', 301, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    -- 视觉理解
    ('qwen3-vl-plus', 'Qwen3 VL Plus', '视觉理解', '[
      ...
      "image",
      "video"
    ]', TRUE, 262144, 32768, '视觉理解 Plus', 400, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen3-vl-flash', 'Qwen3 VL Flash', '视觉理解', '[
      "text",
      "image",
      "video"
    ]', TRUE, 262144, 32768, '视觉理解 Flash', 401, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen-vl-max', 'Qwen VL Max', '视觉理解', '[
      "text",
      "image",
      "video"
    ]', FALSE, 131072, 32768, '视觉旗舰', 402, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    -- 推理专精
    ('qwq-plus', 'QwQ Plus', '推理专精', '[
      "text"
    ]', TRUE, 131072, 8192, '深度推理', 500, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen-deep-research', 'Qwen Deep Research', '推理专精', '[
      "text"
    ]', FALSE, 1000000, 32768, '深度研究', 501, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    -- 全模态
    ('qwen3.5-omni-plus', 'Qwen3.5 Omni Plus', '全模态', '[
      "text",
      "image",
      "video",
      "audio"
    ]', FALSE, 262144, 65536, '全模态 Plus', 600, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),
    ('qwen3.5-omni-flash', 'Qwen3.5 Omni Flash', '全模态', '[
      "text",
      "image",
      "video",
      "audio"
    ]', FALSE, 262144, 65536, '全模态 Flash', 601, TRUE, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000,
     EXTRACT(EPOCH FROM NOW())::BIGINT * 1000)
...
-- 初始化内置技能数据
INSERT INTO agent_desk.skills (id, name, description, author, version, category, tags, icon, bg_color, sys_prompt,
                               max_iters, tools, builtin, user_id, created_at, updated_at)
VALUES ('file-analyzer', '文件分析专家',
        '文件分析专家。擅长读取和分析上传的文件：CSV结构解析、代码审查、配置文件校验、日志分析。当用户上传文件并需要分析时调用。',
        'AgentDesk', '1.0.0', 'file', '[
    "文件",
    "分析"
  ]', 'FileSearch', '#F3F4F6',
   ...
   请用中文回答，分析要具体、有条理。',
        5, '[
    "FileTools"
  ]', TRUE, NULL, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000, EXTRACT(EPOCH FROM NOW())::BIGINT * 1000),

       ('writer', '写作助手', '写作助手。擅长邮件撰写、文本润色、周报生成、技术文档编写。当用户需要写作相关帮助时调用。',
        'AgentDesk', '1.0.0', 'writing', '[
         "写作",
         "邮件",
         "文档"
       ]', 'FileEdit', '#F3F4F6',
   ...
   请用中文回答，代码块使用合适的语言标记。',
       ...
       ('data-analyst', '数据分析师',
        '数据分析师。擅长数据统计、趋势分析、异常检测、生成分析结论。当用户上传数据并需要分析洞察时调用。',
        'AgentDesk', '1.0.0', 'data', '[
         "数据",
         "统计",
         "分析"
       ]', 'BarChart3', '#F3F4F6',
...
-- 17. 项目实体与本地运行集 (见 db/migration-project-runtime.sql)
...
-- 18. AgentDesk canonical memory system
